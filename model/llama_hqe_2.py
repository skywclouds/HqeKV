...
from quant.hqe_1 import KV_cache_hqe_1
from quant.hqe_matmul import qkv_matmul_hqe_1, qkv_matmul_hqe_1_serial, qkv_matmul_hqe_V_serial
...
from transformers.utils import (
    can_return_tuple, LossKwargs, logging,)
...
class LlamaAttention_hqe(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.is_causal = True
        # self.ratio_e_0 = torch.tensor([0.0010969043431821074], dtype=torch.float32)
        self.ratio_e_0 = torch.tensor([0.02], dtype=torch.float32)
        self.ratio_e = self.ratio_e_0.clone()
        # self.ratio_q = torch.tensor([0.10815969754217251, 0.6766178117166646, 0.21412558639798082], dtype=torch.float32)
        self.ratio_q = torch.tensor([0.62, 0.36, 0], dtype=torch.float32)
        self.token_num = 0
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_value is not None:
            ...
            # 计算attention
            start_time = time.time()
            attn_weights, attn_output = qkv_matmul_hqe_1_serial(query_states, past_key_value, attention_mask, self.head_dim, quant_strategy)
            end_time = time.time()
        ...
        # 存储最近的几个注意力分数
        if attn_weights.shape[2] == last_token_num or self.current_length >=13:
        ...
        if self.current_length >= 32:
            ...
            # 前 几 个和后 几 个不量化
            # if sort_importance.shape[0] <= 20:
            #     sort_importance[:] = torch.finfo(token_importance.dtype).max
            #     V_token_norm_importance[:] = torch.finfo(token_importance.dtype).max
            # else:
            #     sort_importance[:4] = torch.finfo(token_importance.dtype).max
            #     sort_importance[-16:] = torch.finfo(token_importance.dtype).max
            #     V_token_norm_importance[:4] = torch.finfo(token_importance.dtype).max
            #     V_token_norm_importance[-16:] = torch.finfo(token_importance.dtype).max

            past_key_value = KV_cache_hqe_1(past_key_value, compress_length, self.history_reserve_length,
