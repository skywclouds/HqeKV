...
from transformers.models.llama.modeling_llama import (
    LlamaRMSNorm, LlamaRotaryEmbedding, rotate_half, apply_rotary_pos_emb,
    LlamaMLP, repeat_kv, LlamaPreTrainedModel)

from quant.CompressUtils import MixedPrecisionCompressUnion
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, compress_config=None, layer_idx: int = 0):
        super().__init__()
        self.config = config
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: LlamaConfig, compress_config=None, layer_idx: int=0):
        ...
        self.self_attn = LlamaAttention(config=config, compress_config=compress_config, layer_idx=layer_idx)
...
class LlamaModel(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig, compress_config=None):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, compress_config=compress_config, layer_idx=layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, compress_config):
        super().__init__(config)
        self.model = LlamaModel(config, compress_config=compress_config)
        self.vocab_size = config.vocab_size
