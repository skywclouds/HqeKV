...
*/
__global__ void bgemv_kernel_uniform_outlier_8(
        ...
        for (int ic_1 = 0; ic_1 < pack_factor; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            if (is_outlier){
              ...
              // q中的第ic_0个数与K中的第ic_0个int32中的第ic_1个int8的相乘
              // 这样一个小循环结束之后实现了
              // q中的第ic_0个数与K中的第ic_0个int32中的4个int8的相乘
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_uniform_outlier_4(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int bit, const int nh_q, const int nh_kv){
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_uniform_outlier_2(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int bit, const int nh_q, const int nh_kv){
        ...
        for (int ic_1 = 0; ic_1 < pack_factor; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            if (is_outlier){
              ...
              // 进行反量化
              float dequantized_weight = cur_scale * cur_single_weight_fp + cur_zero;
              // q中的第ic_0个数与K中的第ic_0个int32中的第ic_1个int2的相乘
              // 这样一个小循环结束之后实现了
              // q中的第ic_0个数与K中的第ic_0个int32中的16个int2的相乘
              psum[ic_1] += dequantized_weight * cur_inp;
            }
            // 进行右移,unpack下一个数
            cur_packed_weight = cur_packed_weight >> bit;
...
__global__ void bgemv_kernel_uniform_outlier_1(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int bit, const int nh_q, const int nh_kv){
        ...
        for (int ic_1 = 0; ic_1 < pack_factor; ic_1++){
          ...
          if (oc_idx < OC){
            ...
            if (is_outlier){
              ...
              // 进行反量化
              float dequantized_weight = cur_scale * cur_single_weight_fp + cur_zero;
              // q中的第ic_0个数与K中的第ic_0个int32中的第ic_1个int1的相乘
              // 这样一个小循环结束之后实现了
              // q中的第ic_0个数与K中的第ic_0个int32中的32个int1的相乘
              psum[ic_1] += dequantized_weight * cur_inp;
            }
            // 进行右移,unpack下一个数
            cur_packed_weight = cur_packed_weight >> bit;
...
Notes:
  ...
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_uniform_group_outlier_8(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
...
__global__ void bgemv_kernel_uniform_group_outlier_4(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int group_size, const int bit, const int nh_q, const int nh_kv){
    const int pack_factor = 32 / 4;// 一个int32存储pack_factor个量化后的数
...
__global__ void bgemv_kernel_uniform_group_outlier_2(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int group_size, const int bit, const int nh_q, const int nh_kv){
    const int pack_factor = 32 / 2;// 一个int32存储pack_factor个量化后的数
...
__global__ void bgemv_kernel_uniform_group_outlier_1(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  half* _outliers, int64_t* _outliers_idx, half* _outputs, const int IC, const int OC, 
  const int group_size, const int bit,const int nh_q, const int nh_kv){
    const int pack_factor = 32 / 1;// 一个int32存储pack_factor个量化后的数
    ...
    const int nh_kv)
{
    ...
    if(bit == 8){
      bgemv_kernel_uniform_outlier_8<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 4){
      bgemv_kernel_uniform_outlier_4<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 2){
      bgemv_kernel_uniform_outlier_2<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 1){
      bgemv_kernel_uniform_outlier_1<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
  ...
  const int nh_kv)
{
  ...
  // 进行核函数计算
  if (bit == 8){
    bgemv_kernel_uniform_group_outlier_8<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
    ...
    bgemv_kernel_uniform_group_outlier_4<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
    ...
    bgemv_kernel_uniform_group_outlier_2<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
    ...
    bgemv_kernel_uniform_group_outlier_1<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, zeros, scaling_factors, outliers, outliers_idx, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
