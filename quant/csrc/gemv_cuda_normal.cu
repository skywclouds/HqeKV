...
*/
__global__ void bgemv_kernel_normalize_8(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_normalize_4(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int bit, const int nh_q, const int nh_kv){
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_normalize_2(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int bit, const int nh_q, const int nh_kv){
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_normalize_1(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int bit, const int nh_q, const int nh_kv){
...
__global__ void bgemv_kernel_normalize_group_8(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int group_size, const int bit, const int nh_q, const int nh_kv){
    ...
    for (int k=0; k < (IC + TILE_DIM - 1) / TILE_DIM; k++){
      ...
      for (int i=0; i<4; i++){
        if (weight_offset + i < OC * ICR / pack_factor)
          qw[i] = *(weight + weight_offset + i);
        if (std_mn_offset + i < OC * ICR / group_size){
          cstd[i] = *(stds + std_mn_offset + i);
          cmean[i] = *(means + std_mn_offset + i);}
        if (inputs_ptr_delta + i < ICR)
          inp[i] = *(inputs + inputs_ptr_delta + i);
...
Notes:
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void bgemv_kernel_normalize_group_4(
    const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
    const int IC, const int OC, const int group_size, const int bit, const int nh_q, const int nh_kv){
      const int pack_factor = 32 / 4;// 一个int32存储pack_factor个量化后的数
      const int batch_idx = blockIdx.x;// 当前线程在第几个batch
...
__global__ void bgemv_kernel_normalize_group_2(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int group_size, const int bit, const int nh_q, const int nh_kv){
    const int pack_factor = 32 / 2;// 一个int32存储pack_factor个量化后的数
    const int batch_idx = blockIdx.x;// 当前线程在第几个batch
...
__global__ void bgemv_kernel_normalize_group_1(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int group_size, const int bit, const int nh_q, const int nh_kv){
    const int pack_factor = 32 / 1;// 一个int32存储pack_factor个量化后的数
    const int batch_idx = blockIdx.x;// 当前线程在第几个batch
...
__global__ void bgemv_kernel_qk_group_token_1(
  const half* _inputs, const uint32_t* _weight, const half* _means, const half* _std, const float* _normal_quantiles_center, half* _outputs, 
  const int IC, const int OC, const int group_size, const int bit, const int head_ratio){
    ...
    const uint32_t*  weight = _weight + _batch_idx * OC * IC / pack_factor;
    const half* stds = _std + _batch_idx * OC * IC / group_size;
    const half* means = _means + _batch_idx * OC * IC / group_size;
    const int num = 0xFF >> (8-bit);
    float res = 0;
    if (oc_idx < OC){ // 检查行是否越界
      for (int k=0; k < (packed_IC + 3) / 4; k++){
        ...
        int std_mn_offset = oc_idx * IC / group_size + group_idx;
        
        uint32_t cur_packed_weight = *(weight + weight_offset);
        float cur_std = __half2float(*(stds+std_mn_offset));
        float cur_mean = __half2float(*(means+std_mn_offset));

        #pragma unroll
        for (int i = 0; i < pack_factor; i++){
          float cur_inp = __half2float(*(inputs+inputs_start_offset+i));
          int cur_weight = cur_packed_weight & num;
          float center = *(_normal_quantiles_center + cur_weight);
          float dequantized_weight = cur_std * center + cur_mean;
  ...
  const int nh_kv)
{
    ...
    if(bit == 8){
      bgemv_kernel_normalize_8<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 4){
      bgemv_kernel_normalize_4<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 2){
      bgemv_kernel_normalize_2<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    }else if(bit == 1){
      bgemv_kernel_normalize_1<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, bit, nh_q, nh_kv);
    ...
    const int nh_kv)
{
    ...
    // 进行核函数计算
    if (bit == 8){
      bgemv_kernel_normalize_group_8<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
        ...
        bgemv_kernel_normalize_group_4<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
        // constants
        num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
      ...
      bgemv_kernel_normalize_group_2<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
      ...
      bgemv_kernel_normalize_group_1<<<num_blocks, num_threads>>>(
      // pointers
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
      // constants
      num_in_channels, num_out_channels, group_size, bit, nh_q, nh_kv
  ...
  const int nh_kv)
{
  ...
  half* means = reinterpret_cast<half*>(_means.data_ptr<at::Half>());
  half* stds = reinterpret_cast<half*>(_stds.data_ptr<at::Half>());
  ...
  if(bit == 1){
    bgemv_kernel_qk_group_token_1<<<num_blocks, num_threads>>>(
      in_feats, kernel, means, stds, normal_quantiles_center, out_feats,
