...
def K_high_2_eviction(high: torch.Tensor, high_idx: torch.Tensor, 
               high_scale_std: torch.Tensor | None, high_mn_mean: torch.Tensor | None,
               high_bit: int,
               mask: torch.Tensor, block_size: int = 32):
    ...
    if high_scale_std is not None:
        ...
        scale_std_mn_mean_mask = scale_std_mn_mean_mask.view(1, 1, 1, -1).expand(bsz, num_heads, head_dim, -1)
    ...
    mask = mask.view(1, 1, 1, -1).expand(bsz, num_heads, head_dim, -1)
...
def V_high_2_eviction(high: torch.Tensor, high_idx: torch.Tensor, 
               high_scale_std: torch.Tensor | None, high_mn_mean: torch.Tensor | None,
               mask: torch.Tensor):
...
def compress_K(sixteen_K, sixteen_K_idx, eight_K, eight_K_idx, eight_K_scale_std, eight_K_mn_mean,
               four_K, four_K_idx, four_K_scale_std, four_K_mn_mean, two_K, two_K_idx, two_K_scale_std, two_K_mn_mean,
               one_K, one_K_idx, one_K_scale_std, one_K_mn_mean, block_size, 
    ...
    return (sixteen_K, sixteen_K_idx, eight_K, eight_K_idx, eight_K_scale_std, eight_K_mn_mean,
               four_K, four_K_idx, four_K_scale_std, four_K_mn_mean, two_K, two_K_idx, two_K_scale_std, two_K_mn_mean,
               one_K, one_K_idx, one_K_scale_std, one_K_mn_mean, block_size, 
               new_sixteen_K_idx, new_eight_K_idx, new_four_K_idx, new_two_K_idx, new_one_K_idx, new_e_K_idx, 
               new_K, new_token_idx)

def compress_V(sixteen_V, sixteen_V_idx, eight_V, eight_V_idx, eight_V_scale_std, eight_V_mn_mean,
               four_V, four_V_idx, four_V_scale_std, four_V_mn_mean, two_V, two_V_idx, two_V_scale_std, two_V_mn_mean,
               one_V, one_V_idx, one_V_scale_std, one_V_mn_mean, group_size, 
               new_sixteen_V_idx, new_eight_V_idx, new_four_V_idx, new_two_V_idx, new_one_V_idx, new_e_V_idx, 
               new_V, new_token_idx, V_token_norm, last_weights, V_quantize_high, V_dequantize_high, V_quantize_low, V_dequantize_low):
    # 将各部分 V 应丢弃的丢弃
    ...
    return (sixteen_V, sixteen_V_idx, eight_V, eight_V_idx, eight_V_scale_std, eight_V_mn_mean,
               four_V, four_V_idx, four_V_scale_std, four_V_mn_mean, two_V, two_V_idx, two_V_scale_std, two_V_mn_mean,
               one_V, one_V_idx, one_V_scale_std, one_V_mn_mean, group_size, 
               new_sixteen_V_idx, new_eight_V_idx, new_four_V_idx, new_two_V_idx, new_one_V_idx, new_e_V_idx, 
               new_V, new_token_idx, V_token_norm)
...
def KV_cache_hqe_2(
    ...
    # 不能被32整除的部分不作处理
    residual_new_K = new_K[:, :, compress_length:, :].clone() if new_K is not None else None
    residual_new_V = new_V[:, :, compress_length:, :].clone() if new_V is not None else None
    ...
    # 统计 1 -> 2, 2 -> 4, 4 -> 16 的个数
    jsonl_path = '/home/users/wanghe/projects/kv_hq_1-main/tests/low_2_high.jsonl'
    low_2_high_dict = {
        'K' : {'1 -> 2' : 0, '2 -> 4' : 0, '4 -> 16' : 0},
        'V' : {'1 -> 2' : 0, '2 -> 4' : 0, '4 -> 16' : 0},
        'length' : compress_length + history_length if not isinstance(history_length, torch.Tensor) else compress_length + history_length.item()
        }
    ...
    # if new_sixteen_K_idx is not None and four_K_idx is not None:
    #     four_2_sixteen = torch.isin(four_K_idx, new_sixteen_K_idx)
    #     low_2_high_dict['K']['4 -> 16'] = four_2_sixteen.sum().item()
    ...
    # if new_sixteen_V_idx is not None and four_V_idx is not None:
    #     four_2_sixteen = torch.isin(four_V_idx, new_sixteen_V_idx)
    #     low_2_high_dict['V']['4 -> 16'] = four_2_sixteen.sum().item()
    ...
    if new_sixteen_V_idx is not None:
        ...
        if new_token_idx is not None:
            ...
            if new_2_sixteen.any():
                ...
                new_2_sixteen_V_idx = new_token_idx[new_2_sixteen]
                new_2_sixteen = new_2_sixteen.view(1, 1, -1, 1).expand_as(new_V)
                new_2_sixteen_V = new_V[new_2_sixteen].clone().view(bsz, num_heads, -1, num_channel)
                sixteen_V = torch.cat((sixteen_V, new_2_sixteen_V), dim=2) if sixteen_V is not None else new_2_sixteen_V
                sixteen_V_idx = torch.cat((sixteen_V_idx, new_2_sixteen_V_idx)) if sixteen_V_idx is not None else new_2_sixteen_V_idx
    # 丢弃之后更新各部分的索引
    if new_e_V_idx is not None:
...
def KV_cache_hqe_21(
    ...
    if V_token_range is None:
        V_token_norm = calculate_token_range(new_V, V_token_norm)
    else:
        V_token_norm = V_token_range
    V_token_norm_importance = token_importance * V_token_norm
    ...
    (sixteen_K, sixteen_K_idx, eight_K, eight_K_idx, eight_K_scale_std, eight_K_mn_mean,
        four_K, four_K_idx, four_K_scale_std, four_K_mn_mean, two_K, two_K_idx, two_K_scale_std, two_K_mn_mean,
        one_K, one_K_idx, one_K_scale_std, one_K_mn_mean, block_size, 
        new_sixteen_K_idx, new_eight_K_idx, new_four_K_idx, new_two_K_idx, new_one_K_idx, new_e_K_idx, 
        new_K, new_token_idx) = compress_K(sixteen_K, sixteen_K_idx, eight_K, eight_K_idx, eight_K_scale_std, eight_K_mn_mean,
               four_K, four_K_idx, four_K_scale_std, four_K_mn_mean, two_K, two_K_idx, two_K_scale_std, two_K_mn_mean,
               one_K, one_K_idx, one_K_scale_std, one_K_mn_mean, block_size, 
               new_sixteen_K_idx, new_eight_K_idx, new_four_K_idx, new_two_K_idx, new_one_K_idx, new_e_K_idx, 
               new_K, new_token_idx, K_quantize_high, K_dequantize_high, K_quantize_low, K_dequantize_low)
    ...
    (sixteen_V, sixteen_V_idx, eight_V, eight_V_idx, eight_V_scale_std, eight_V_mn_mean,
        four_V, four_V_idx, four_V_scale_std, four_V_mn_mean, two_V, two_V_idx, two_V_scale_std, two_V_mn_mean,
        one_V, one_V_idx, one_V_scale_std, one_V_mn_mean, group_size, 
        new_sixteen_V_idx, new_eight_V_idx, new_four_V_idx, new_two_V_idx, new_one_V_idx, new_e_V_idx, 
        new_V, new_token_idx, V_token_norm) = compress_V(sixteen_V, sixteen_V_idx, eight_V, eight_V_idx, eight_V_scale_std, eight_V_mn_mean,
               four_V, four_V_idx, four_V_scale_std, four_V_mn_mean, two_V, two_V_idx, two_V_scale_std, two_V_mn_mean,
               one_V, one_V_idx, one_V_scale_std, one_V_mn_mean, group_size, 
               new_sixteen_V_idx, new_eight_V_idx, new_four_V_idx, new_two_V_idx, new_one_V_idx, new_e_V_idx, 
               new_V, new_token_idx, V_token_norm, last_weights, V_quantize_high, V_dequantize_high, V_quantize_low, V_dequantize_low)
...
def KV_cache_reconstruct_hqe(
    ...
    if eight_V is not None:
        reconstuct_V[:, :, eight_V_idx, :] = V_dequantize_high(eight_V, eight_V_scale_std, eight_V_mn_mean, group_size, 8)
    ...
    if eight_V is not None:
        ...
        reconstuct_V[:, :, four_V_idx, :] = reconstuct_V_eight
...
if __name__ == '__main__':
    ...
    past_key_value = KV_cache_hqe_2(past_key_value, compress_length, histroy_length, block_idx, K_bit_num, V_bit_num, token_importance, quant_strategy, last_weights)
