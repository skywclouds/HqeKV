...
def compress_K(sixteen_K, sixteen_K_idx, eight_K, eight_K_idx, eight_K_scale_std, eight_K_mn_mean,
               ...
               new_sixteen_K_idx, new_eight_K_idx, new_four_K_idx, new_two_K_idx, new_one_K_idx, 
...
def compress_V(sixteen_V, sixteen_V_idx, eight_V, eight_V_idx, eight_V_scale_std, eight_V_mn_mean,
               ...
               new_sixteen_V_idx, new_eight_V_idx, new_four_V_idx, new_two_V_idx, new_one_V_idx,
...
if __name__ == '__main__':
    ...
    w = torch.matmul(q, new_K.transpose(2, 3))
    w /= 128 ** 0.5
    w = torch.softmax(w, dim=-1)
    o = torch.matmul(w, new_V)
    ...
    block_bit_num = torch.tensor([192, 192, 192, 192, 256], dtype=torch.int32, device=device)
    token_bit_num = torch.tensor([200, 200, 200, 200, 224], dtype=torch.int32, device=device)

    past_key_value = KV_cache_compress(past_key_value, compress_length, histroy_length, block_idx, token_idx, block_bit_num, token_bit_num, 'uniform_group')
    ...
    block_idx += add
    block_idx = block_idx.flatten()
    block_bit_num = torch.tensor([192, 0, 384, 192, 320], dtype=torch.int32, device=device)
    token_bit_num = torch.tensor([200, 0, 400, 200, 288], dtype=torch.int32, device=device)

    past_key_value = KV_cache_compress(past_key_value, compress_length, histroy_length, block_idx, token_idx, block_bit_num, token_bit_num, 'uniform_group')
    for item in past_key_value:
        print(item.shape if isinstance(item, torch.Tensor) else item)

    reconstruct_K, reconstruct_V = KV_cache_reconstruct(past_key_value, quant_strategy)
