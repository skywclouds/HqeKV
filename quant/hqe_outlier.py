from model.public_func import (calculate_token_norm, calculate_new_token_range,
    calculate_token_range, get_block_token_idx)
import torch
...
def KV_cache_hqe_outlier(
    ...
    if len(reserve_V_idx) > 0:
        ...
        outliers = outliers[:, :, reserve_V_idx, :]
        outliers_idx = outliers_idx[:, :, reserve_V_idx, :]
...
def KV_cache_reconstruct_hqe_outlier(
        past_key_value: tuple, 
        quant_strategy: str,
        block_size: int = 32,
        group_size: int = 32
...
if __name__ == '__main__':
    ...
    past_key_value = KV_cache_hqe_outlier(past_key_value, compress_length, histroy_length, block_idx, K_bit_num, V_bit_num, token_importance, quant_strategy)
    ...
    past_key_value = KV_cache_hqe_outlier(past_key_value, compress_length, histroy_length, block_idx, K_bit_num, V_bit_num, token_importance, quant_strategy)
    reconstruct_K, reconstruct_V = KV_cache_reconstruct_hqe_outlier(past_key_value, quant_strategy)
